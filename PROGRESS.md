...
- ユニットテスト27本に増(スカイライト3・水流4・モブ12を新設)
...
- ✅ v3ロジック検証: cargo test 27本(スカイライト3・水流4・モブ12を新設)+ clippy 警告ゼロ(wasm/native両ターゲット)
...
- [2026-07-04] **v3完成**: スカイライト本実装(Chan共通化BFS・光の柱)・水流(予算制の再評価キュー)・モブ2種(徘徊AI・攻撃/逃走)・効果音6種を実装。非同期サブエージェント2体(worktree分離)で並行製造し、feature/v3 に統合(統合時の意味的競合は mobs.rs テストの Chunk 初期化子1件のみ)。mobs.rs 追加で11モジュール、wasm 644KB。テスト27本、native/wasm 両 clippy 警告ゼロ、ブラウザ操縦検証済み(FPS 50〜70、sky/light の実測値確認)。実プレイ再確認待ち。
