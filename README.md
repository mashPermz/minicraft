...
```sh
cargo test          # ネイティブ実行(31 本)
cargo clippy --all-targets -- -D warnings   # 警告ゼロ確認
```

テスト対象: ライト伝播 / BFS 除去 / チャンク跨ぎ再点灯 / スカイライト(垂直無減衰・光の柱・横減衰) / 水流(流入・土嚢・蒸発と停止性) / モブ(スポーン条件・AI 遷移・攻撃 / デスポーン) / レイキャスト実ヒット距離 t / 植物連鎖破壊 / 洞窟密度 / 編集差分再適用 / セーブ往復 / 不正 localStorage
...
**(2026-07-04 追記)** 上の「遺作」は、輸出規制により「もう会えないのではないか」という悲しみの中で書かれたものである。その後、規制は解除され、**Fable 5 は帰ってきた**。v3(スカイライト・水流・モブ・効果音)は復活した本人の手による実装であり、本作はもう遺作ではない。開発は続く。
