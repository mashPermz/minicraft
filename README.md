...
**wasm 624KB / 外部アセットゼロ / 静的サーバならどこでも動く**
...
| `world.rs`    | チャンク生成・編集・洞窟・ライティング BFS |
...
- チャンクごとに `light: Vec<u8>` (0–15)
- BFS 拡散(TORCH_LIGHT=14)、除去・再伝播
...
```sh
cargo test          # ネイティブ実行(8 本)
cargo clippy --all-targets -- -D warnings   # 警告ゼロ確認
```

テスト対象: ライト伝播 / BFS 除去 / チャンク跨ぎ再点灯 / 植物連鎖破壊 / 洞窟密度 / 編集差分再適用 / セーブ往復 / 不正 localStorage
